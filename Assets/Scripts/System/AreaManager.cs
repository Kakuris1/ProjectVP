...
public class AreaManager : MonoBehaviour
{
    ...
    private List<GameObject> _activeEnemies     // 스폰된 적 추적 리스트
        = new List<GameObject>();               
    private Transform _playerTransform;         // 플레이어 참조
    private bool _isSpawned = false;            // 현재 스폰되어 있는지 상태 플래그
    private Transform _enemyContainer;          // 스폰 된 적을 담을 컨테이너
    ...
    {
        // [ 적 컨테이너 생성]
        // 이 AreaManager가 MainLevelScene에 있으므로, 이 컨테이너도 MainLevelScene에 생성
        _enemyContainer = new GameObject($"[Area {areaNumber} Enemies]").transform;

        // 씬 하이어라키를 깔끔하게 하기 위해
        // 컨테이너를 EnemyContainer의 자식으로 설정
    ...
    private void SpawnEnemiesByType(int enemyType, int count)
    {
        for (int i = 0; i < count; i++)
        {
            ...
            // GPP에 스폰된 Instance의 부모를 MainLevelScene에 있는
            // _enemyContainer로 설정하여 씬을 '이동'시킴
